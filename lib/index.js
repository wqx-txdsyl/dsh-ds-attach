...
async function extractPdf(buf) {
  ...
  text = text.trim();
  // Run the vision OCR when the PDF has no text layer (pure scan) OR its text
  // layer looks like a low-quality OCR layer (scanned book with embedded OCR
  // text: page-number/header noise, replacement chars, foreign alphabets in a
  // CJK document). A clean born-digital text layer is used as-is (fast path).
  // DS_ATTACH_FORCE_OCR=1 always OCRs; DS_ATTACH_TEXTLAYER_ONLY=1 never does.
 ...
 * - No text layer at all (pure scan) → yes.
 * - Text layer present but noisy → yes (low-quality embedded OCR layer).
 * - Clean born-digital text layer → no (fast path, keep the layer).
 * Heuristics: embedded OCR layers typically carry page-number/header garbage
 * like " - 12 - " / " - CC,? - " between paragraphs, replacement chars, and
 * stray Greek/Cyrillic/etc. alphabets inside CJK text. Thresholds are tuned so
 * ordinary typeset PDFs (which rarely exceed 2-3 such fragments) stay on the
 * fast path.
 */
function shouldOcr(text, textless) {
  const force = process.env.DS_ATTACH_FORCE_OCR === "1";
  const textLayerOnly = process.env.DS_ATTACH_TEXTLAYER_ONLY === "1";
  if (textLayerOnly) return false;
  if (force) return true;
  if (textless || !text || text.trim() === "") return true;
  const len = text.length;
  if (len < 50) return true;
  // " - xxx - " / "－xxx－" fragments (page numbers, folios, stray OCR junk)
  const pageNoise = (text.match(/[-－—]\s*[A-Za-z0-9?×,./]{1,10}\s*[-－—]/g) || []).length;
  // Replacement chars + non-CJK scripts mixed into (presumably CJK) text
  const bad = (text.match(/[\uFFFD\u0370-\u03FF\u0400-\u04FF\u0E00-\u0E7F\u0600-\u06FF\u2190-\u2BFF]/g) || []).length;
  return pageNoise >= 5 || bad / len > 0.005;
}
