...
/** Extract text from ODF documents (odt/ods/odp): text:p / text:h paragraphs of content.xml. */
async function extractOpenDoc(buf) {
  const jszip = loadLib("jszip");
  if (!jszip) return "";
  let zip;
  ...
  if (!f) return "";
  let xml;
  try { xml = await f.async("string"); } catch { return ""; }
  const texts = [];
  const re = /<(?:text:p|text:h)(?=[ >])[^>]*>([\s\S]*?)<\/(?:text:p|text:h)>/g;
  let m;
  while ((m = re.exec(xml))) {
    const t = m[1]
      .replace(/<[^>]+>/g, "") // strip inline spans / annotations
...
/** Extract readable text from RTF (rough: decodes \uN/\'xx escapes, strips
 *  control words and group braces, keeps visible characters). */
function extractRtf(buf) {
  ...
  s = s.replace(/\\fonttbl[^}]*\}/g, ""); // drop font table
...
async function extractText(buf, ext) {
  ...
  if (e === "xlsx" || e === "xls") return { text: await extractXlsx(buf), extracted: true };
  if (e === "pptx") return { text: await extractPptx(buf), extracted: true };
  if (e === "odt" || e === "ods" || e === "odp") return { text: await extractOpenDoc(buf), extracted: true };
  if (e === "rtf") return { text: extractRtf(buf), extracted: true };
