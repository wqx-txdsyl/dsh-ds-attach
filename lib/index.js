 ...
 *   scanned book with embedded OCR text — page-number/header garbage like
 *   " - 12 - " / " - CC,? - ", replacement chars, stray non-CJK alphabets).
 * - Clean born-digital text layer → no (fast path).
 * DS_ATTACH_FORCE_OCR=1 always OCRs; DS_ATTACH_TEXTLAYER_ONLY=1 never does.
 * Note: OCR on a long scan runs per-page through the vision model and can
 * take several minutes — that's the tradeoff for clean text.
 */
