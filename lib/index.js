...
const ROUTE_DELETE = "/ds-attach/delete";
...
async function handleDelete(req, res) {
  try {
    ...
    let body;
    try { body = JSON.parse(raw.toString("utf8")); } catch { body = null; }
    const p = body && typeof body.path === "string" ? body.path : "";
    if (!p) { sendJson(res, 400, { ok: false, error: "missing path" }); return; }
    const root = resolve(uploadRoot());
    const target = resolve(p);
    if (!target.startsWith(root)) { sendJson(res, 403, { ok: false, error: "forbidden" }); return; }
    await unlink(target);
...
function apply(ctx) {
  ...
  ctx.effect(() => {
    const offs = [
      ...
      server.register({ kind: "exact", path: ROUTE_DELETE, handler: handleDelete })
    ];
