window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function UserMessageWithAttachments(props) {
      ...
      // resolveModule returns the MODULE object (like MarkdownText above); take
      // the .ImageGallery member and verify it is a component, otherwise any
      // image message would crash the renderer (which gets abdicated by DSH,
      // silently falling back to the product renderer = raw marker text).
      const ImageGallery = galleryRef.current && typeof galleryRef.current.ImageGallery === 'function'
        ? galleryRef.current.ImageGallery : null;
