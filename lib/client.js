window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const removeBtnStyle = {
      // Hidden by default; revealed on card hover (matches DS chat).
      // Hiding is done in CSS (.ds-attach-remove{opacity:0}) — an inline
      // opacity:0 would beat the hover rule and make the button invisible.
    ...
    function AttachDock(props) {
      ...
      const removeItem = useCallback((id) => {
        ...
        setItems(next);
        persist(next);
        // Delete the stored file on disk too (fire-and-forget; UI already
        // reflects removal even if the request fails).
        if (it && it.path) {
          fetch('/ds-attach/delete', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
    ...
    function UserMessageWithAttachments(props) {
      ...
      return h('div', { style: userRowStyle, 'data-time-hover-root': true },
        h('div', { style: userStackStyle },
          images.length > 0 ? h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'flex-end' } },
            images.map((img, i) => h('div', { key: i, style: { width: '96px', height: '96px', borderRadius: '8px', background: 'var(--dsw-alias-interactive-bg-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--dsw-alias-label-tertiary)', fontSize: '11px' } }, '🖼️'))
          ) : null,
    ...
    const CSS = [
      ...
      // Delete button hidden by default; appears on card hover (matches DS chat)
      '.ds-attach-remove{opacity:0}',
