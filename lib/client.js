window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function FileCard(props) {
      ...
      const statusText = st === 'uploading' ? '上传中…' : st === 'parsing' ? '解析中…' : st === 'error' ? '上传失败' : st === 'empty' ? '未提取到文本' : (item.truncated ? '（已截断）' : fmtSize(item.size));
      ...
      useEffect(() => {
        injectRef.current = () => {
          ...
          const blocks = list.map((it) => {
            const body = it.text.length > 40000 ? it.text.slice(0, 40000) + '\n…[内容已截断]' : it.text;
            return `【附件】${it.name}\n【文件大小】${it.size || 0}\n【文件内容】\n${body}\n【文件内容结束】`;
          });
          const addition = '\n\n' + blocks.join('\n\n');
    ...
    const CSS = [
      ...
      '.ds-attach-card:hover button{opacity:1}',
      '@media (prefers-reduced-motion:reduce){.ds-attach button{transition:none}}'
    ].join('\n');
